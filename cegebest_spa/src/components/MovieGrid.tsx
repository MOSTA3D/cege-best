...
function MovieGrid() {
  const [popularMoviesList, setPopularMoviesList]: [
    MovieType[] | never,
    Dispatch<SetStateAction<MovieType[] | never[]>>
  ] = useState([] as MovieType[]);
  ...
  useEffect(() => {
    ...
    (async function () {
      const data = await fetchFacad.getData<MovieType[]>(
        `${BACKEND_URL}/popular?page=1`
      );
    ...
    window.onscroll = () => {
      ...
      if (
        ...
        (async function () {
          const data = await fetchFacad.getData(
            `${BACKEND_URL}/popular?page=${pageRef.current}`
          );
          if ((data as Err).message) {
